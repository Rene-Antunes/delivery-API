...
public class SpringFoxConfig {
	...
	public Docket apiDocket() {
		...
		return new Docket(DocumentationType.OAS_30)
				.select()
				.apis(RequestHandlerSelectors.basePackage("com.delivery.food.api"))
	...
	private List<Response> globalPostPutResponseMessages() {
		return Arrays.asList(
				...
				.description("Requisição recusada porque o corpo está em um formato diferente não suportado")
				.representation(MediaType.APPLICATION_JSON)
				.apply(getProblemaModelReference())
				.build()
	...
	private Consumer<RepresentationBuilder> getProblemaModelReference(){
		return r -> r.model(m -> m.name("Problema")
				.referenceModel(ref -> ref.key(k -> k.qualifiedModelName(
						q -> q.name("Problema").namespace("com.delivery.food.api.exceptionhandler")))));
	}
